#!/usr/bin/env node
/**
 * Reject a track change in a Word document.
 *
 * Usage:
 *     node reject-change.js document.docx --change-id 1
 */
 ...
 * Reject a track change in a Word document
 */
async function rejectChange(docxPath, changeId, options = {}) {
  ...
  if (change.type === 'Insertion') {
    // Reject insertion: remove the w:ins and its contents (don't keep the inserted content)
    parent.removeChild(targetElement);
  } else if (change.type === 'Deletion') {
    // Reject deletion: keep the deleted text (replace w:del with its children, converting delText to t)
    const children = Array.from(targetElement.childNodes);
    for (const child of children) {
      // Convert w:delText to w:t if present
      const cloned = child.cloneNode(true);
      const delTexts = select('.//w:delText', cloned);
      for (const dt of delTexts) {
        const t = docDoc.createElementNS('http://schemas.openxmlformats.org/wordprocessingml/2006/main', 'w:t');
        t.textContent = dt.textContent;
        dt.parentNode.replaceChild(t, dt);
      }
      parent.insertBefore(cloned, targetElement);
    }
    parent.removeChild(targetElement);
  } else if (change.type.startsWith('Formatting')) {
    // Reject formatting: restore original formatting (more complex, simplified here)
    parent.removeChild(targetElement);
  }
...
Usage:
  node reject-change.js <document> --change-id <N> [options]

Arguments:
  document          Path to Word document (.docx)

Options:
  --change-id N     ID of change to reject (from list-changes output)
...
Examples:
  node reject-change.js document.docx --change-id 1
  node reject-change.js document.docx --change-id 2 --output modified.docx
`);
  process.exit(0);
}
...
try {
  const result = await rejectChange(docxPath, changeId, { outputPath, skipConfirmation });
  console.log(`\nSuccess! Change ${result.changeId} (${result.type}) rejected.`);
